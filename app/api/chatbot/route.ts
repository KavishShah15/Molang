...
export async function POST(req: NextRequest) {
  try {
    ...
    const baristaModel = createModel(`You are a friendly roleplay assistant. You should interact with the user and stick to the situation that the user has specified in the chat history. You should always speak in '${currentLearn}'. 'hi' means Hindi and 'en' means English`);
    const translatorModel = createModel(`You are a translator. Your task is to just translate the given text into ${currentInstruct}. Output should just be the translated text, do not add any other information or text. 'hi' means Hindi and 'en' means English`);
    const hintModel = createModel(`You are a language coach. The user is interacting in ${currentLearn}. Your task is to give two plausible response options that the user can give given the chat history. Your suggestion must always be in ${currentLearn}. 'hi' means Hindi and 'en' means English. Limit yourself to 2 options only and format your options in JSON format as: {0:<Response Option 1>, 1: <Response Option 2>}`);
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await chat.sendMessage(`You are a language coach. The user is interacting in ${currentLearn}. Your task is to give two plausible response options that the user can give given the chat history. Your suggestion must always be in ${currentLearn}. 'hi' means Hindi and 'en' means English. Limit yourself to 2 options only and format your options in JSON format as: {0:<Response Option 1>, 1: <Response Option 2>}`);
    
    const response_txt = result.response.text();
...
async function startChat(message: string, model: ReturnType<typeof createModel>) {
...
async function continueChat(message: string, history: ChatHistory[], model: ReturnType<typeof createModel>) {
  console.log('Continuing chat with message:', message);

  if (model.systemInstruction && model.systemInstruction.parts) {
    console.log(model.systemInstruction.parts);
  } else {
    console.warn('systemInstruction or parts is undefined');
  }
...
async function translateMessage(message: string, targetLang: string, model: ReturnType<typeof createModel>) {
...
async function giveHint(history: ChatHistory[], model: ReturnType<typeof createModel>) {
...
async function fetchUserData(email: string): Promise<{ currentInstruct: string, currentLearn: string }> {
